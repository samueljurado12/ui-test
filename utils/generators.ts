...
export const selectRandomFromArray = <T>(arr: T[]): T => {
  const randomIndex = loremIpsum.generator.generateRandomInteger(
    0,
    arr.length - 1
  );
...
const generateRandomAddress = (): Address => ({
  address:
    loremIpsum.generateWords(2) +
    loremIpsum.generator.generateRandomInteger(10, 100),
...
const generateRandomPhoneNumber = (): string => {
  ...
  const centralOfficeCode = loremIpsum.generator.generateRandomInteger(
    100,
    999
  );
...
export const generateRandomUser = (): UserData => {
  ...
  return {
    ...
    password:
      loremIpsum.generateWords(3).replaceAll(" ", "_") +
      loremIpsum.generator.generateRandomInteger(1000, 9999),
